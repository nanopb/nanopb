...
separate protoc plugin, `protoc-gen-nanopb-validate`, which also writes
`*_validate.h` / `*_validate.c` and injects `filter_udp()` / `filter_tcp()` into
the generated `.pb.h` / `.pb.c`. See :doc:`validation` for how to invoke it.
