...
class MangleNames:
    ...
    def create_name(self, names, include_in_map = True):
        '''Create name for a new message / enum.
        Argument can be either string or Names instance.
        If include_in_map is True, a mapping define is created from the original name.
        '''
...
class ProtoFile:
    ...
    def generate_header(self, includes, headername, options):
        ...
        header_basename = os.path.basename(headername)
        hdrname_symbol = make_identifier(str(self.manglenames.create_name(header_basename, False)))
        hdrname_symbol_noext = make_identifier(str(self.manglenames.create_name(header_basename.split('.')[0], False)))
        yield '#ifndef %s_INCLUDED\n' % hdrname_symbol
        yield '#define %s_INCLUDED\n' % hdrname_symbol
        ...
        if self.messages:
            ...
            for identifier, msize in messagesizes:
                if msize is not None:
                    ...
                    if identifier == max_messagesize[0]:
                        guards[cpp_guard].add('#define %-40s %s' % (
                            Globals.naming_style.define_name(hdrname_symbol + "_MAX_SIZE"), Globals.naming_style.define_name(identifier)))
            ...
            if [msg for msg in self.messages if hasattr(msg,'msgid')]:
                ...
                yield '#define %s \\\n' % Globals.naming_style.define_name(hdrname_symbol_noext + "_MESSAGES")
